...
export default class SecretKey {
	...
	/**
	 * Create a new SecretKey operator.
	 *
	 * @param pleskApi - The Plesk API instance this is associated with.
	 */
	...
	public async create(ipAddress: string, description?: string) {
		const response = await this.pleskApi.execute("secret_key", [
			"-c",
			"-ip-address",
			ipAddress,
	 ...
	 */
	public async list() {
		const response = await this.pleskApi.execute("secret_key", [ "-l" ]);
	...
	public async delete(key: number | string) {
		await this.pleskApi.execute("secret_key", [
			"-d",
			"-key",
